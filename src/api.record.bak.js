...
async function removeBasicDOMShit(dom) {
	...
	for (const node of new Array(...dom.querySelector("body").children)) {
	...
	dom.querySelector("footer").remove();
	dom.querySelector(".title.m_10").remove();
	dom.querySelector(".m_t_5.m_b_10.t_l.f_0").remove();
}
...
function parseFcTypeFromUrl(url) {
    ...
    const match = url.match(/playlog\/(\w+)\.png/);
    if (match) {
        const fcType = match[1].toUpperCase();
...
async function getBasicRecords() {
	...
	for (const node of dom.children) {
        ...
        const musicId = parseMusicIdFromUrl(basicBlock.querySelector(".music_img").src);
        const musicKind = parseMusicKindFromUrl(basicBlock.querySelector(".playlog_music_kind_icon").src);
        ...
        const imgs = playResultInnerBlock.querySelectorAll("img");
        const fcType = parseFcTypeFromUrl(imgs[0].src);
        const syncPlayType = parseSyncPlayTypeFromUrl(imgs[1].src);
        const syncPlayRank = parseSyncPlayRankFromUrl(imgs.length > 2 ? imgs[2].src : null);

        const isDeluxscoreNewRecord = scoreBlock.querySelector(".playlog_deluxscore_newrecord") !== null;
        const deluxscoreTexts = scoreBlock.querySelector("div").innerText.replaceAll(',', "").split(" / ");
        const deluxscoreMax = parseInt(deluxscoreTexts[1]);
        const deluxscore = parseInt(deluxscoreTexts[0]);
